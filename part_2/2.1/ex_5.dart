...
Future<void> main() async {
  ...
  // Создаем "медленного" подписчика, который использует pause/resume
  // для гарантии последовательной обработки событий.
  late StreamSubscription<int> subscription;
  subscription = controller.stream.listen(
    (event) async {
      // Немедленно ставим на паузу, чтобы не пропустить события
      subscription.pause();
      ...
      // Возобновляем подписку для получения следующего события
      subscription.resume();
    },
    onDone: () {
